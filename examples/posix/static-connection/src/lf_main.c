...
int main() {
    ...
    // CircularBuffer trigger_buffers[trigger_buffers_size];
    ...
    inst_t schedule_0[] = {
      ...
      // Line 3: JAL: store return address in Worker 0's RETURN_ADDR and jump to SYNC_BLOCK
      // {.func=execute_inst_JAL, .opcode=JAL, .op1.reg=(reg_t*)&(state->return_addr), .op2.imm=WORKER_0_SYNC_BLOCK, .op3.imm=0},
      // Line 4: JAL: store return address in ZERO and jump to INIT
      ...
      // WORKER_0_INIT:
      // Line 5: Execute function envs[simpleconnection_main].reaction_array[0]->function with argument envs[simpleconnection_main].reactor_self_array[1]
      ...
      // Line 6: Increment Worker 0's COUNTER by adding Worker 0's COUNTER and 1LL
      {.func=execute_inst_ADDI, .opcode=ADDI, .op1.reg=(reg_t*)&(state->counter), .op2.reg=(reg_t*)&(state->counter), .op3.imm=1LL},

      // Use ADD to clear the is_present field of the out port.
      {.func=execute_inst_ADD, .opcode=ADD, .op1.reg=(reg_t*)&(main_reactor.source->out->super.super.is_present), .op2.reg=(reg_t*)&(state->zero), .op3.reg=(reg_t*)&(state->zero)},
      
      // Call DelayConnection_cleanup to get to the schedule_at in the scheduler.
      ...
      // Advance time for sink.reaction_0


      
      // WORKER_0_EXECUTE_SimpleConnection_sink_reaction_1_7ed46f7f:
      // Line 24: Execute function envs[simpleconnection_main].reaction_array[2]->function with argument envs[simpleconnection_main].reactor_self_array[2]
      {.func=execute_inst_EXE, .opcode=EXE, .op1.reg=(reg_t*)(main_reactor.sink->reaction0.super.body), .op2.reg=(reg_t*)&((main_reactor.sink->reaction0.super)), .op3.reg=(reg_t*)&(main_reactor.source)},
      // WORKER_0_EXECUTE_SimpleConnection_sink_reaction_2_f52ba644:
      // Line 31: Execute function envs[simpleconnection_main].reaction_array[3]->function with argument envs[simpleconnection_main].reactor_self_array[2]
      {.func=execute_inst_EXE, .opcode=EXE, .op1.reg=(reg_t*)(main_reactor.sink->reaction1.super.body), .op2.reg=(reg_t*)&((main_reactor.sink->reaction1.super)), .op3.reg=(reg_t*)&(main_reactor.source)},
