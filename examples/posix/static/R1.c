...
LF_DEFINE_REACTION_CTOR(Reactor_R1, reaction0, 0, NULL, NULL);



LF_DEFINE_REACTION_CTOR(Reactor_R1, reaction1, 1, NULL, NULL);
...
LF_REACTOR_CTOR_SIGNATURE_WITH_PARAMETERS(Reactor_R1 , OutputExternalCtorArgs *_out_external  ) {
   ...
   LF_INITIALIZE_REACTION(Reactor_R1, reaction0, NEVER); 
     LF_STARTUP_REGISTER_EFFECT(self->reaction0);
           
           
   LF_INITIALIZE_REACTION(Reactor_R1, reaction1, NEVER); 
