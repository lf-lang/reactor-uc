...
LF_DEFINE_REACTION_CTOR(Reactor_R2, reaction0, 0, NULL, NULL);



LF_DEFINE_REACTION_CTOR(Reactor_R2, reaction1, 1, NULL, NULL);



LF_DEFINE_REACTION_CTOR(Reactor_R2, reaction2, 2, NULL, NULL);
...
LF_REACTOR_CTOR_SIGNATURE_WITH_PARAMETERS(Reactor_R2 , InputExternalCtorArgs *_in_external , int expected ) {
   ...
   LF_INITIALIZE_REACTION(Reactor_R2, reaction0, NEVER); 
     LF_STARTUP_REGISTER_EFFECT(self->reaction0);
           
           
   LF_INITIALIZE_REACTION(Reactor_R2, reaction1, NEVER); 
     LF_PORT_REGISTER_EFFECT(self->in, self->reaction1, 1);
           
           
   LF_INITIALIZE_REACTION(Reactor_R2, reaction2, NEVER); 
