...
LF_DEFINE_STARTUP_COORDINATOR_STRUCT(Federate, 2, 11);
LF_DEFINE_STARTUP_COORDINATOR_CTOR(Federate, 2, 1, 11, 5, JOIN_IMMEDIATELY);

LF_DEFINE_SHUTDOWN_COORDINATOR_STRUCT(Federate, 9);
LF_DEFINE_SHUTDOWN_COORDINATOR_CTOR(Federate, 1, 9);
...
typedef struct {
  ...
  LF_DEFINE_SHUTDOWN_COORDINATOR(Federate);
...
LF_REACTOR_CTOR_SIGNATURE(MainSender) {
  ...
  LF_INITIALIZE_SHUTDOWN_COORDINATOR(Federate);
