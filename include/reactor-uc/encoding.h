...
int encode_protobuf(const PortMessage *message, char *buffer, size_t buffer_size) {
  // turing write buffer into pb_ostream buffer
  pb_ostream_t stream_out = pb_ostream_from_buffer((pb_byte_t *)buffer, buffer_size);
...
int decode_protobuf(PortMessage *message, const char *buffer, size_t buffer_size) {
  pb_istream_t stream_in = pb_istream_from_buffer((const pb_byte_t *)buffer, buffer_size);
