...
class UcGeneratorFederated(context: LFGeneratorContext, scopeProvider: LFGlobalScopeProvider) :
    ...
    override fun doGenerate(resource: Resource, context: LFGeneratorContext) {
        ...
        if (
            clockSyncMainState.state != UcClockSyncMainState.OFF &&
                federates.filter { it.clockSyncParams.grandmaster }.isEmpty()
        ) {
            messageReporter
                .nowhere()
                .warning(
                    "No clock sync grandmaster specified. Selecting first federate as grandmaster."
                )
        ...
        for (ucFederate in federates) {
            ...
            val res =
                doGenerateFederate(ucFederate.inst.eResource()!!, context, srcGenPath, ucFederate)
