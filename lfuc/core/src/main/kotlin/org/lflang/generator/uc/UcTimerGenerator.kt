...
class UcTimerGenerator(private val reactor: Reactor) {
  private fun generateSelfStructs(timer: Timer) =
      "LF_DEFINE_TIMER_STRUCT(${reactor.codeType}, ${timer.name}, ${reactor.getEffects(timer).size}, ${reactor.getObservers(timer).size});"

  private fun generateCtor(timer: Timer) =
      "LF_DEFINE_TIMER_CTOR(${reactor.codeType}, ${timer.name}, ${reactor.getEffects(timer).size}, ${reactor.getObservers(timer).size});"
  ...
  private fun generateReactorCtorCode(timer: Timer) =
      "LF_INITIALIZE_TIMER(${reactor.codeType}, ${timer.name}, ${timer.offset.toCCode()}, ${timer.period.orNever().toCCode()});"
