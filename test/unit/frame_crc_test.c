...
// Known-answer vectors for CRC-32/ISO-HDLC (a.k.a. IEEE 802.3, zlib lf_crc32).
void test_crc32_empty(void) { TEST_ASSERT_EQUAL_HEX32(0x00000000, lf_crc32((const uint8_t*)"", 0)); }

void test_crc32_check_vector(void) {
  // The standard "check" value: CRC32("123456789") == 0xCBF43926
  const char* s = "123456789";
  TEST_ASSERT_EQUAL_HEX32(0xCBF43926, lf_crc32((const uint8_t*)s, strlen(s)));
}

void test_crc32_single_byte(void) {
  const uint8_t b = 0x00;
  TEST_ASSERT_EQUAL_HEX32(0xD202EF8D, lf_crc32(&b, 1));
}
...
void test_crc32_detects_single_bit_flip(void) {
  ...
  memcpy(b, a, sizeof(a));
  b[7] ^= 0x01;
  TEST_ASSERT_NOT_EQUAL(lf_crc32(a, sizeof(a)), lf_crc32(b, sizeof(b)));
}
